...
- 可将剪贴板文本或图片快速保存为新记录
- 支持窗口置顶，边看边记
- 支持每日笔记：一键打开/创建以当日日期命名的记录
- 自定义标题栏与内容同色（浅/深主题自动同步），菜单收进标题栏「☰」下拉

### 2. 富文本编辑
- 支持标题、小标题、正文、说明等文本类型
- 支持粗体、斜体、删除线、下划线、行内代码、高亮
- 支持引用、分隔线、有序列表、无序列表、任务列表
- 支持表格（列宽可拖）、图片（拖拽插入 + 缩放）、链接
- 支持代码块（语言选择 + 一键复制 + 语法高亮）
- 支持 LaTeX 公式块（实时预览）
- 支持 Callout 提示块（信息 / 提示 / 警告 / 危险四类型可切换）
- 支持字体、颜色、自定义颜色
- 支持“焦点”和“块”两种文本装饰样式
- 支持折叠块大纲：`Tab` / `Shift+Tab` 缩进、键盘导航，可导出为层级化 Markdown 列表结构
- 支持斜杠命令 `/` 快速插入各类块（方向键选择、回车确认）
- 支持笔记间 `[[` 互链与编辑器底部反向链接面板（链接提及 + 未链接提及）
- 支持查找、替换、全文字数统计和阅读时间估算

### 3. 记录管理
- 支持标签与文件夹
- 支持置顶、收藏、归档、回收站
- 支持命令面板 `Ctrl + P`：跳转笔记、切换视图、执行命令、插入模板
- 支持模板：文件夹为「模板」的记录可一键插入，自动替换 `{{title}}` / `{{date}}` / `{{time}}`
- 支持最近编辑时间线视图
